...
# Code taken from the official repo: https://github.com/modestyachts/ImageNetV2_pytorch/blob/master/imagenetv2_pytorch/ImageNetV2_dataset.py
...
class ImageNetV2Dataset(Dataset):
    def __init__(self, variant="matched-frequency", transform=None, location="."):
        ...
        if not self.dataset_root.exists() or len(self.fnames) != V2_DATASET_SIZE:
            # Just in case: do not download the zip file. Uncomment if needed
            raise ValueError("ImageNetV2 dataset not found")
            # if not self.tar_root.exists():
